...
GLboolean TrigonometricBernsteinSurface3::CalculatePartialDerivatives(
        GLuint maximum_order_of_partial_derivatives, GLdouble u, GLdouble v, PartialDerivatives &pd) const
{
    ...
    for (GLuint d = 1; d <= maximum_order_of_partial_derivatives; d++)
    {
        ...
        {
            dAu(d, i) = ((i>0) ? _u_c[i] /_u_c[i-1] * (GLdouble)i / sua * dAu(d-1,i-1) : 0.0 ) -
                    (_n - (GLdouble)i) / tua * dAu(d-1,i)
                    -((i < 2 * _n) ? (_u_c[i] /_u_c[i+1] * ( 2.0 * _n - (GLdouble)i ) / sua ) * dAu(d-1, i+1):0.0);

        }
        for(GLuint j = 0; j < v_size; j++)
        {
            dAv(d, j) = ((j>0) ? _v_c[j] /_v_c[j-1] * (GLdouble)j / sva * dAv(d-1,j-1) : 0.0 ) -
                    (_m - (GLdouble)j) / tva * dAv(d-1,j)
                    -((j < 2 * _m) ? (_v_c[j] /_v_c[j+1] * ( 2.0 * _m - (GLdouble)j ) / sva ) * dAv(d-1, j+1):0.0);
        }
    }
    for (GLuint i = 0; i < u_size; ++i)
    {
...
TrigonometricBernsteinSurface3* TrigonometricBernsteinSurface3::Clone()
{
    return new TrigonometricBernsteinSurface3(*this);
}
}
