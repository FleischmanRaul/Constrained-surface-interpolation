...
// done: delete vertex buffer objects of unit normal vectors, texture coordinates, and indices
...
GLboolean TriangulatedMesh3::SaveToOFF(const string &file_name) const           //done
{
    ...
    f << "OFF\n";
    f << this->VertexCount() << " " << this->FaceCount() << " " << 0 << "\n";
...
GLfloat* TriangulatedMesh3::MapTextureBuffer(GLenum access_flag) const
{
    ...
    glBindBuffer(GL_TEXTURE_BUFFER, _vbo_vertices);
    GLfloat* result = (GLfloat*)glMapBuffer(GL_TEXTURE_BUFFER, access_flag);
...
{
    glBindBuffer(GL_TEXTURE_BUFFER, _vbo_tex_coordinates);
    glUnmapBuffer(GL_TEXTURE_BUFFER);
    glBindBuffer(GL_TEXTURE_BUFFER, 0);
}

//GLuint VertexCount() const; // done
GLuint TriangulatedMesh3::VertexCount() const
{
    return _vertex.size();
}
//GLuint FaceCount() const;   // done
