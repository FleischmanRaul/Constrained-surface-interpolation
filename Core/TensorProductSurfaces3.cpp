...
TriangulatedMesh3* TensorProductSurface3::GenerateImage(GLuint u_div_point_count, GLuint v_div_point_count, GLenum usage_flag) const
{
    ...
    PartialDerivatives pd = PartialDerivatives(2);

    for (GLuint i = 0; i < u_div_point_count; ++i)
    {
        GLdouble u = min(_u_min + i * du, _u_max);
        GLfloat  s = min(i * sdu, GLfloat(1.0));
        for (GLuint j = 0; j < v_div_point_count; ++j)
        {
            GLdouble v = std::min(_v_min + j * dv, _v_max);
            GLfloat  t = std::min(j * tdv, GLfloat(1.0));
...
TensorProductSurface3::PartialDerivatives::PartialDerivatives(GLuint maximum_order_of_partial_derivatives):TriangularMatrix<DCoordinate3>(maximum_order_of_partial_derivatives + 1)
{
}
...
GLvoid TensorProductSurface3::PartialDerivatives::LoadNullVectors()
{
    _data(0, 0) = DCoordinate3(0.0, 0.0, 0.0);

    // first ordered partial derivates of the surface
    _data(1, 0) = DCoordinate3(0.0, 0.0, 0.0);
    _data(1, 1) = DCoordinate3(0.0, 0.0, 0.0);

    // second ordered partial derivates of the surface
    _data(2, 0) = DCoordinate3(0.0, 0.0, 0.0);
    _data(2, 1) = DCoordinate3(0.0, 0.0, 0.0);
    _data(2, 2) = DCoordinate3(0.0, 0.0, 0.0);

    // third ordered partial derivates of the surface
    _data(3, 0) = DCoordinate3(0.0, 0.0, 0.0);
    _data(3, 1) = DCoordinate3(0.0, 0.0, 0.0);
    _data(3, 2) = DCoordinate3(0.0, 0.0, 0.0);
    _data(3, 3) = DCoordinate3(0.0, 0.0, 0.0);
}*/
